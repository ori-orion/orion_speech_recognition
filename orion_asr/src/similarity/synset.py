...
def word_similarity(word1, word2):
    ...
    synset_wordone, synset_wordtwo = proper_synset(word1,
                                                   word2)  # cant just add +".n.01" to words to convert them to a synset.
    ...
    for joint_word in joint_word_set:
        ...
        if (joint_word in sentence):
            ...
            sim_word, beta_sim_measure = most_similar_word(joint_word,
                                                           sentence)  # gets the most similar word in that sentence.
...
def compare(candidate_list, sentence=None):
    ...
    for i in range(len(candidate_list)):
        p = synset(candidate_list[i], sentence)
        print(candidate_list[i] + ': ' + str(p))
        if p > max and p > 0.5:
            task = candidate_list[i]
            max = p
    if task == '':
        print('No valid task provided, try again')
        compare(candidate_list)
    print('Task: ' + task)
